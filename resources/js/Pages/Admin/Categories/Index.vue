...
const handleFileChange = (file) => {
    categoryImages.value.push(file)
}
...
const openEditModal = (cat) => {
    ...
    category_image.value = cat.feature_image
...
const addCategory = function () {
    ...
    formData.append('slug', categorySlug.value)
...
const updateCategory = async () => {
    ...
    try {
        ...
        await router.post(route('admin.categories.update', { id: categoryId.value }), formData, {
            onSuccess: (page) => {
                dialogVisible.value = false
                resetFormData()
                Swal.fire({
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                ...
                router.delete(route('admin.categories.destroy', { id: cat.id }), {
                    onSuccess: (page) => {
                        this.delete(cat, index)
                        Swal.fire({
                            toast: true,
                            icon: 'success',
                            position: 'top-end',
            ...
            <el-dialog v-model="dialogVisible" width="700" :before-close="handleClose">
                <section class="bg-white dark:bg-gray-900">
                    <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
                        <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">{{ editMode ? 'Update' :
                            'Add' }}
                            Category</h2>
                        <form @submit.prevent=" editMode ? updateCategory() : addCategory()">
                            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    <input type="text" name="name" id="name" v-model="categorySlug"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="Type category slug" required="">
                                </div>
                            ...
                            <button type="submit"
                                ...
                                {{ editMode ? 'Update Categroy' : 'Add Category' }}
                            </button>
                        </form>
                    </div>
                </section>
                    ...
                    <div
                        ...
                        <div class="flex items-center flex-1 space-x-4">
                            <h5>
                                ...
                                <span class="dark:text-white">{{ categories.length }}</span>
                            </h5>
                        </div>
                        <div
                            ...
                            <tbody>
                                <tr v-for="cat in categories" :key="cat.id"
                                    ...
                                    <th scope="row"
                                        ...
                                        <img :src="`../category_images/${cat.feature_image}`" class="w-auto h-8 mr-3">
                                        {{ cat.name }}
                                    </th>
                                    <td class="px-4 py-3 items-center">
                                        <button :id="`${cat.id}-dropdown-button`"
                                            :data-dropdown-toggle="`${cat.id}-dropdown`"
